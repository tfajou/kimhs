...
def wrap_async(f):
    ...
    def g(self, loop, *args, **kwargs):
        ...
        if it is None:
            ...
            return result
        # print(".futures.py.wrap_async {}\t{}".format(result, result.exc_info()))
        ...
        def callback(fut):
            try:
                if fut is None:
                    ...
                    # print("44444==wrap_async callback {}\t{}".format(fut, it))
                    exc_info = fut.exc_info()
                    if exc_info:
                        x = it.throw(*exc_info)
                        # print("444440==wrap_async callback {}\t{}}".format(x, *exc_info))
                    else:
                        x = it.send(fut.result())
                        # print("444441==wrap_async callback {}\t{}".format(x, fut.result))
            except StopIteration:
                # print("444442==wrap_async callback StopIteration")
                result.set_result(None)
            except BaseException:
                # print("444443==wrap_async callback BaseException")
                result.set_exc_info(sys.exc_info())
            else:
                if isinstance(x, Result):
                    # print("444444==wrap_async callback ExceptionElse{}\t{}".format(x, x.value))
                    result.set_result(x.value)
                else:
                    x.add_done_callback(callback)
                    # print("444445==wrap_async callback ExceptionElse{}".format(x))
